...
<body>
    ...
    <div>
        <form action="index.php?menuop=eventoPass" method="post">
   ...
   <tbody>
       ...
       while($dados= mysqli_fetch_assoc($rs)){ 
             if( $dataAtual >=$dados["DataInicial"]){
       ?>
...
echo '<a href="?menuop=eventoPass&pagina=1"> primeira pagina </a>';
if($pagina>6){
 ?>
  <a href="?menuop=eventoPass$pagina=<?php echo $pagina-1?>"> << </a>;
 <?php
...
for ($i=1;$i<=$totalpagina;$i++){
    if($i>=($pagina-5) && $i <=($pagina+5)){
        if($i==$pagina){
           ...
           echo "<a href=\"?menuop=eventoPass&pagina=$i\">$i</a>";
        }
...
echo "<a href=\"?menuop=eventoPass&pagina=$totalpagina\">ultima pagina</a>";
